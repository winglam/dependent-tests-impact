...
public class SubjectProgramsInfoFigureGenerator {
	...
	public static void main(String[] args) {
        ...
        for (String subjectInfo : subjectInfoList) {
        	...
        	if (normalizedStr.charAt(0) == '#') {
        		...
        		// formal name of subject, informal name of subject, directory containing src, 
        		// directory containing test, directory containing bin or classes, first revision, 
        		// second revision, directory containing src of new version, directory containing test of new version
        ...
        for (ProjectSubjectProgramInfo subject : subjects) {
			...
			subject.addAutoTests(FileTools.parseFileToList(new File(subject.getRootDir() + File.separator + subject.getInformalName() + "-auto-order")).size());
			subject.addOrigTests(FileTools.parseFileToList(new File(subject.getRootDir() + File.separator + subject.getInformalName() + "-orig-order")).size());
	...
	private static int getDiff(String experimentsDir, File dir, File newDir, String clocFile) {
		int loc = 0;
    	try {
			ProcessBuilder pb = new ProcessBuilder(clocFile, "--diff", dir.getCanonicalPath(), newDir.getCanonicalPath(), "--csv");
			pb.directory(new File(experimentsDir));
			pb.redirectErrorStream(true);
	        BufferedReader br = new BufferedReader(new InputStreamReader(pb.start().getInputStream()));
	        ...
	        for (String s : outputs) {
	        	...
	        	if (splitComma.length == 18) {
	        		if (splitComma[0].equals("Java")) {
	        			...
	        			loc += Math.max(Integer.parseInt(splitComma[15].trim()), Integer.parseInt(splitComma[16].trim()));
	        		}
	        	} else {
	        		System.err.println("Malformed line from CLOC. Line: " + s);
	        	}
