...
public class MaybeAssertRule extends MyAbstractJUnitRule {
				...
				for (ASTMethodDeclaration method : node
					...
					if (method.getBlock() != null && isJUnitMethod(method, data) && !isIgnored(method)) {
						...
						if (!allConstructors
							...
							if (homeMadeAssertionsWithMaybes.contains(method) || isMaybe(method) ||  callsHomeMadeMaybeAssertion(method)) {
				...
				if (prefix.jjtGetNumChildren() > 0) {
					...
					if(methodName!=null){
						...
						if (statement != null && statement.getFirstDescendantOfType(ASTAssignmentOperator.class) == null) {
							...
							if (!containsMethod && !isThirdPartyAssertion(callName)
								/*	
									&& (callName.contains("assert")
									|| callName.contains("check")
									|| callName.contains("verify")
									|| callName.contains("expect") 
									|| callName.contains("test")) */   ) {
								return true;
							}
						}
					}
					...
					if (prefix.usesSuperModifier()) {
						...
						if (suffixes.size() > 1) {
							...
							//String methodName = suffix.getImage().toLowerCase();
						/*
							if (suffixes.get(1).jjtGetNumChildren() > 0) {
								if(methodName.contains("assert")
										|| methodName.contains("check")
										|| methodName.contains("verify")
										|| methodName.contains("expect") 
	...
	private boolean callsHomeMadeMaybeAssertion(ASTMethodDeclaration method) {
		...
		for (ASTStatementExpression statement : method
			...
			if (prefix != null) {
				ASTName name = prefix.getFirstChildOfType(ASTName.class);
				if (name != null) {
					for (ASTMethodDeclaration assertionMethod : homeMadeAssertionsWithMaybes) {
						if (name.hasImageEqualTo(assertionMethod
								.getMethodName())) {
							return true;
						}
