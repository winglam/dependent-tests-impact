...
public class TestsShouldNotCallOtherTestsRule extends MyAbstractJUnitRule {
			...
			if (isJUnit3Class || isJUnit4Class) {
					...
					for (NameOccurrence occurrence : list) {
						...
						if (!method.equals(declaration.jjtGetParent())
							...
							for(Call call: checkForTransitivity){
								if(isJUnitMethod(call.getFinalMethod(), data) && !isIgnored(call.getFinalMethod())){
				...
				for(Call call: calls){
					...
					for (VariableNameDeclaration field : fields) {
						for (NameOccurrence fieldOccurrence : variableDeclarations
								.get(field)) {
							ScopedNode location = fieldOccurrence
											.getLocation();
							Node exp = location.getNthParent(2);
							Node statement = location.getNthParent(3);
							if(exp!=null && statement!=null && statement.jjtGetNumChildren()>1 && statement.jjtGetChild(0).equals(exp) 
									&& statement.jjtGetChild(1).equals(statement.getFirstChildOfType(ASTAssignmentOperator.class))){											
								ASTMethodDeclaration methodRelatedToField = getOuterMethodDeclaration(location);
								
								
								List<ASTMethodDeclaration> chain = call.getChain();
								for(ASTMethodDeclaration callerMethod: chain){
									if (callerMethod.equals(methodRelatedToField)) {
										isViolation = true;
									}
								}
							}
					...
					if (isViolation) {
						addViolation(data, call.getMethodCall());
					}
				}
			}
	...
	private Set<Call> checkForTransitivity(
		...
		if (!set.contains(method)) {
			...
			if(declarations.containsKey(new MethodNameDeclaration(methodDeclarator))){
	...
	private class Call{
		...
		private ASTMethodDeclaration test;
		public ASTMethodDeclaration getFinalMethod() {
			return test;
		}
