  ...
  for k in "${testTypes[@]}"; do
    ...
    for i in "${coverages[@]}"; do
      for j in "${seleOrders[@]}"; do
        ...
        java -Xms1g -Xmx2g -cp ${3}/*:${4}/*:${5}:${6} edu.washington.cs.dt.impact.Main.OneConfigurationRunner -technique selection -coverage $i -order $j -origOrder ${8}/${1}-$k-order -testInputDir ${8}/sootTestOutput-$k-selection -filesToDelete ${2}/${1}-env-files -project "${1}" -testType $k -oldVersCFG ${8}/selectionOutput -newVersCFG ${2}/selectionOutput -getCoverage -outputDir ${7} -timesToRun 1
